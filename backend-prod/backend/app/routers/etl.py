...
def normalize_excel_df(df, bank_code: str) -> List[Dict[str, Any]]:
    ...
    if not has_date or not has_desc:
        ...
        logger.warning(f"Column mapping failed for {bank_code}. Looking for date in {date_cols}, desc in {desc_cols}")
        logger.warning(f"Available columns (normalized): {list(df_cols_normalized.keys())}")
